...
Ejecuta Jest con soporte nativo de ES Modules (`--experimental-vm-modules`) y `--forceExit` para cerrar el pool de PostgreSQL al finalizar. Los tests truncan las tablas antes de cada caso y siembran datos frescos.
...
Conecta tu cuenta de Railway al repositorio de GitHub (`FFigueroa26/ProyectoM2_FelixFigueroa`) y crea un nuevo proyecto desde el repositorio.
...
Agrega un servicio **PostgreSQL** desde el dashboard de Railway; la variable `DATABASE_URL` se inyecta automáticamente.

### 4. Internal URL vs Public URL

- **Internal URL:** `postgresql://postgres:...@postgres.railway.internal:5432/railway` — usada entre servicios dentro del mismo proyecto Railway. No expuesta a internet.
- **Public URL:** Se genera automáticamente cuando Railway asigna un dominio (`https://proyectom2.up.railway.app`). Railway expone el puerto de la app web automáticamente.

### 5. Setup de base de datos en Railway

Conéctate via Railway CLI o desde el dashboard (conectando con los datos de `DATABASE_URL`) y ejecuta:
...
Railway provee SSL requerido; el pool ya está configurado con `ssl: { rejectUnauthorized: false }` para manejar el certificado autofirmado.
...
Verificada con `GET /health` — respuesta:

```json
{"status":"ok","timestamp":"2026-06-26T04:54:54.975Z"}
```

<img src="https://i.ibb.co/VWvZ305j/Captura-de-pantalla-2026-06-26-002539.png" alt="Deploy en Railway" width="800">

---

## Registro del uso de AI en el proyecto

Durante el desarrollo de este proyecto se utilizaron herramientas de inteligencia artificial como apoyo en las siguientes tareas:
...
**Influencia:** El asistente generó el archivo `docs/openapi.json` con todos los endpoints, schemas y códigos de respuesta, que luego se sirve vía Swagger UI Express en `/api-docs`.
...
**Influencia:** El asistente recomendó usar `TRUNCATE TABLE ... RESTART IDENTITY CASCADE` dentro de un `beforeEach` y cerrar el pool con `afterAll`. A partir de ahí se estructuraron los tests manualmente cubriendo los casos obligatorios.
...
**Influencia:** El asistente sugirió hacer una consulta `SELECT` después del DELETE para verificar que el recurso fue eliminado, patrón que se aplicó en los tests de eliminación.
